...
// Create the 'EvolutionChamber' for the aliens
local oldAlienTeamSpawnInitialStructures = AlienTeam.SpawnInitialStructures
function AlienTeam:SpawnInitialStructures(techPoint)

    local tower, hive = oldAlienTeamSpawnInitialStructures(self, techPoint)

	local evochamber = CreateEntityForTeam(kTechId.EvolutionChamber, hive:GetOrigin(), self:GetTeamNumber(), nil)
	if evochamber then
		local origin = evochamber:GetOrigin()
		origin.y = origin.y + 3
		evochamber:SetOrigin(origin)
		self.evochamber = evochamber
	end
...
function Hive:LoginPlayer(player)
	...
	if team and team.GetEvolutionChamber then
		local evochamber = team:GetEvolutionChamber()
		if evochamber then
			commander.evochamber = evochamber:GetId()
		end
	end
	
end

function AlienTeam:GetEvolutionChamber()
