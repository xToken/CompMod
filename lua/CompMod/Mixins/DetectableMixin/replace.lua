...
-- lua\CompMod\Mixins\DetectableMixin\replace.lua
-- - Dragon
...
local function UpdateSensorBlip(self)
    ...
    if not self:GetIsDetected() or not alive or (self.GetShowSensorBlip and not self:GetShowSensorBlip()) then
        ...
        if not blip then
        	...
        	local blipType = SensorBlip.kMapName
        	if self.SensorBlipType then
        		blipType = self:SensorBlipType()
        	end
            blip = CreateEntity(blipType)
